...
/datum/gamemode/dynamic
	...
	var/datum/dynamic/stat/dynamic_mode/dynamic_stats = null
...
//NTODO	usr.client.holder.check_antagonists() // Refreshes the window
...
/datum/gamemode/dynamic/proc/show_threatlog(mob/admin)
	...
	if(!check_rights(R_ADMIN, 0, admin) && (SSticker.current_state != GAME_STATE_FINISHED))
		return
...
/proc/makeBody(var/mob/observer/G_found) // Uses stripped down and bastardized code from respawn character
	...
	new_character.change_name("random")
