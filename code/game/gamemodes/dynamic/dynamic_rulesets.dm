...
/datum/dynamic_ruleset/proc/send_applications(var/list/possible_volunteers = list())
	...
	for(var/mob/M in possible_volunteers)
		...
		to_chat(M, "[logo ? "[icon2base64(logo_icon)]" : ""]<span class='recruit'>The mode is looking for volunteers to become [initial(role_category.id)]. (<a href='?src=\ref[src];signup=\ref[M]'>Apply now!</a>)</span>[logo ? "[icon2base64(logo_icon)]" : ""]")
	...
	spawn(1 MINUTES)
		...
		for(var/mob/M in possible_volunteers)
			if(!M.client || jobban_isbanned(M, role_category) || M.client.is_afk())
				continue
			to_chat(M, "[logo ? "[icon2base64(logo_icon)]" : ""]<span class='recruit'>Applications for [initial(role_category.id)] are now closed.</span>[logo ? "[icon2base64(logo_icon)]" : ""]")
...
/datum/dynamic_ruleset/roundstart/trim_candidates()
	...
	for(var/mob/new_player/P in candidates)
		if (!P.client || !P.mind || !P.mind.assigned_role)//are they connected?
			...
			a++
			continue
		if (!P.client.desires_role(role_pref) || jobban_isbanned(P, role_id) || (role_category_override && jobban_isbanned(P, role_category_override)))//are they willing and not antag-banned?
			candidates.Remove(P)
			b++
			continue
