...
/datum/faction/proc/Dismantle()
	for(var/datum/role/R in members)
		var/datum/gamemode/G = SSticker.mode
		G.orphaned_roles += R
		members -= R
	qdel(objective_holder)
	var/datum/gamemode/dynamic/D = SSticker.mode
	D.factions -= src
	qdel(src)
...
/datum/faction/Topic(href, href_list)
	...
	if(href_list["destroyfac"])
		if(!check_rights(R_ADMIN, 0, usr.client))
...
//			emergency_shuttle.online = 1
			OnPostDefeat()
			security_state.set_security_level("blue")
		//	SSticker.StopThematic()
		if(FACTION_ENDGAME) //Faction is nearing victory. Set red alert and play endgame music.
			//SSticker.StartThematic("endgame")
			sleep(2 SECONDS)
			security_state.set_security_level("red")
		...
		if(R.antag && R.antag.current && R.antag.current.client && R.antag.GetRole(R.id))//if the mind doesn't have access to the role, they shouldn't see the icons
			for(var/datum/role/R_target in members)
				if(R_target.antag && R_target.antag.current)
					...
					var/image/I = image('icons/mob/mob.dmi', loc = imageloc, icon_state = hud_icon)
					I.pixel_x = 20
					I.pixel_y = 20
					I.plane = ON_MOB_HUD_LAYER
